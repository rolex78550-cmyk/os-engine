...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    app.get("*", (req, res) => {
      // Static SEO pages generated by scripts/build-seo-pages.mjs (dist/<slug>/index.html)
      const clean = req.path.replace(/\/+$/, "");
      if (/^\/[a-z0-9-]+$/.test(clean)) {
        const staticPage = path.join(distPath, clean.slice(1), "index.html");
        if (fs.existsSync(staticPage)) return res.sendFile(staticPage);
      }
